...
export default function VoiceCallSettingsPopover({ voiceChat, t, disabled = false, onOpenSettings }: Props) {
  ...
  const [activeCategory, setActiveCategory] = useState<"model" | "voice" | "translation">("model");

  const rootRef = useRef<HTMLDivElement>(null);

  // Sync activeProvider to current voiceChatProvider when opened
  useEffect(() => {
    if (open) {
      setActiveProvider(voiceChat.voiceChatProvider);
      setActiveCategory("model");
    }
  }, [open, voiceChat.voiceChatProvider]);
  ...
  function handleToggle() {
    if (!open && rootRef.current) {
      ...
      setFlyoutToLeft(rect.left + 620 > window.innerWidth);
        ...
        >
          ...
          <div className="vsVoiceLevel1List">
            {voiceChat.voiceChatRealtimeChoicesByProvider.map((group) => {
              ...
              const isActiveProvider = group.provider === (activeProvider || voiceChat.voiceChatProvider);
              return (
                <button
                  key={group.provider}
                  type="button"
          ...
          {/* LEVEL 2: CATEGORY SELECTION MENU (Flies out to the right/left of Level 1) */}
          ...
          {/* LEVEL 3: SPECIFIC CONTENT PANEL (Flies out to the right/left of Level 2) */}
          <div className={`vsVoiceLevel3Flyout${flyoutToLeft ? " flyLeft" : ""}`}>
            ...
            {activeCategory === "translation" && isLiveTranslateModel ? (
              <div className="vsVoiceSettingsSection vsVoiceTranslationCard" style={{ width: 280 }}>
